...
namespace OAuthService
{
    public class MicrosoftConnect
    {
        ...
        public MicrosoftConnect(string consumerKey, string consumerSecret, string scope = "User.Read", string authorizeUrl = "https://login.microsoftonline.com/common/oauth2/v2.0/authorize", string accessTockenUrl = "https://login.microsoftonline.com/common/oauth2/v2.0/token", string userJsonUrl = "https://graph.microsoft.com/v1.0/users")
        {
            _apiKey = consumerKey;
        ...
        private string AuthenticationUrl
        {
            get
            {
                ...
                queryParameters.Add(new QueryParameter("response_mode", Const.MicrosoftResponseMode));
                queryParameters.Add(new QueryParameter("scope", _scope));
        ...
        public User FetchProfile()
        {
            ...
            if (userJson["value"][0].ContainsKey("userPrincipalName") == false) { throw new Exception("we could not fetch your email address from OAuth Token. Please make sure that your profile is up to date with email address."); }
            return new User { Email = userJson["value"][0]["userPrincipalName"], Name = userJson["value"][0]["displayName"], Token = App.TokenSecret, AuthRouter = "Microsoft" };
        }
